...
struct TABLE_META { // gry: 内存对齐，实际写盘占用 24 个字节
  ...
  uint32_t id; // 表 id, 按序号生成的, 序号从0开始, 类似于列数字, 参看 tianmu_data 目录下的数字表名.列索引
...
class TableShare final {
 ...
 private:
  ...
  std::shared_ptr<TianmuTable> current; // gry: 当前 tianmu 表快照,后续再用的时候会取，如果是第一取，则赋值tianmu table指针
