...

{
  ...
  DEBUG_ASSERT(virt_col[attr_no] || a.term_.IsNull());  // the second possibility is count(*)
  ...
  switch (a.oper_type_) {
    ...
    case common::ColOperation::COUNT:
      if (a.term_.IsNull() || (!ag_distinct && virt_col[attr_no]->IsConst())) {
  ...
  TIANMU_LOG(LogCtl_Level::DEBUG,
             ...
             attr_no, static_cast<int>(input_mode[attr_no]), a.alias_, a.si_.separator.c_str(), a.si_.order,
             static_cast<int>(ag_type), ag_size);

  gt.AddAggregatedColumn(virt_col[attr_no], ag_oper, ag_distinct, ag_type, ag_size, ag_prec, ag_collation,
                         a.si_);  // note: size will be automatically calculated for all numericals
