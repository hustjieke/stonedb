  ...
  switch (TypeName()) {
    ...
    case common::CT::MEDIUMINT:  // 24-bit
      if (val == common::NULL_VALUE_64)
        ...
        ((AttrBuffer<int> *)buffer)->Set(obj, SDB_INT_MIN);
      else
    ...
    case common::CT::SMALLINT:  // 16-bit
      if (val == common::NULL_VALUE_64)
        ...
        ((AttrBuffer<short> *)buffer)->Set(obj, SDB_SMALLINT_MAX);
      else if (val == common::MINUS_INF_64)
        ((AttrBuffer<short> *)buffer)->Set(obj, SDB_SMALLINT_MIN);
      else
    ...
    case common::CT::BYTEINT:  // 8-bit
      if (val == common::NULL_VALUE_64)
        ...
        ((AttrBuffer<char> *)buffer)->Set(obj, SDB_TINYINT_MAX);
      else if (val == common::MINUS_INF_64)
        ((AttrBuffer<char> *)buffer)->Set(obj, SDB_TINYINT_MIN);
      else
...
int64_t TempTable::Attr::GetMaxInt64(int pack) {
  ...
  stop = stop > no_obj ? no_obj : stop;
  if (no_materialized < stop) return common::NULL_VALUE_64;
  int64_t val, max = common::SDB_BIGINT_MIN;
  for (int64_t i = start; i < stop; i++) {
    if (!IsNull(i)) {
      val = GetNotNullValueInt64(i);
      if ((ATI::IsRealType(ct.GetTypeName()) && (max == common::SDB_BIGINT_MIN || *(double *)&val > *(double *)&max)) ||
          (!ATI::IsRealType(ct.GetTypeName()) && val > max))
        max = val;
    }
  }
  return (max == common::SDB_BIGINT_MIN ? common::PLUS_INF_64 : max);
}
...
int64_t TempTable::Attr::GetMinInt64(int pack) {
  ...
  stop = stop > no_obj ? no_obj : stop;
  if (no_materialized < stop) return common::NULL_VALUE_64;
  int64_t val, min = common::SDB_BIGINT_MAX;
  for (int64_t i = start; i < stop; i++) {
    if (!IsNull(i)) {
      val = GetNotNullValueInt64(i);
      if ((ATI::IsRealType(ct.GetTypeName()) && (min == common::SDB_BIGINT_MAX || *(double *)&val < *(double *)&min)) ||
          (!ATI::IsRealType(ct.GetTypeName()) && val < min))
        min = val;
    }
  }
  return (min == common::SDB_BIGINT_MAX ? common::MINUS_INF_64 : min);
}
