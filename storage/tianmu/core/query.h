...
class Query final {
 ...
 private:
  ...
  std::map<Item_sdbfield *, int> sdbitems_cur_var_ids;
 ...
 private:
   ...
   * Item_sdbfield items \param item - root of MySQL expression tree to be
  ...
  //	 * substituted with Item_sdbfield items. In case of aggregation it is
  // substituted with its
  // whole subtree by a single
  //	 * Item_sdbfield, however, there is an AddColumn step added to
  // compilation with this
  // aggregation and Item_sdbfield refers to the
