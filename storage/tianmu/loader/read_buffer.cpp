...
bool ReadBuffer::StartReadingThread() {
  if (!read_thread_buffer_) {
    ...
    if (!read_thread_buffer_) {
      TIANMU_LOG(LogCtl_Level::ERROR, "out of memory (%d bytes failed). (42)", size_);
      return false;
    }
  }
    ...
    if (sdb_stream_->IsOpen()) {
      ...
      while (no_steps < 15 && to_read > 0) {
        ...
        if (r != -1) {
          ...
          no_steps++;
          TIANMU_LOG(LogCtl_Level::WARN, "Reading from the input file error: %s", std::strerror(errno));
